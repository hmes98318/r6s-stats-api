...
module.exports = {
    general: async function (platform, name) {
        ...
        stats_general.deaths = checkNumber(
            profile[profile.indexOf('Deaths') + 1]
        )
        stats_general.win_ = profile[profile.indexOf('Win %') + 1]
        stats_general.wins = checkNumber(profile[profile.indexOf('Wins') + 1])
        stats_general.losses = checkNumber(
            profile[profile.indexOf('Losses') + 1]
        )

        stats_general.headshot_ = profile[profile.indexOf('Headshot %') + 1]
        stats_general.headshots = checkNumber(
            profile[profile.indexOf('Headshots') + 1]
        )

        stats_general.time_played = profile[profile.indexOf('Time Played') + 1]
        stats_general.matches_played = checkNumber(
            profile[profile.indexOf('Matches Played') + 1]
        )
        stats_general.total_xp = profile[profile.indexOf('Total XP') + 1]
        stats_general.melee_kills = checkNumber(
            profile[profile.indexOf('Melee Kills') + 1]
        )
        stats_general.blind_kills = checkNumber(
            profile[profile.indexOf('Blind Kills') + 1]
        )
    ...
    casual: async function (platform, name) {
        ...
        stats_casual.deaths = checkNumber(
            profile[profile.indexOf('Deaths') + 1]
        )
        stats_casual.win_ = profile[profile.indexOf('Win %') + 1]
        stats_casual.wins = checkNumber(profile[profile.indexOf('Wins') + 1])
        stats_casual.losses = checkNumber(
            profile[profile.indexOf('Losses') + 1]
        )

        stats_casual.time_played = profile[profile.indexOf('Time Played') + 1]
        stats_casual.matches = checkNumber(
            profile[profile.indexOf('Matches') + 1]
        )
        stats_casual.kills_match = checkNumber(
            profile[profile.indexOf('Kills/match') + 1]
        )
        stats_casual.kills_min = checkNumber(
            profile[profile.indexOf('Kills/min') + 1]
        )
    ...
    rank: async function (platform, name) {
        ...
        stats_rank.matches = checkNumber(
            profile[profile.indexOf('Matches') + 1]
        )
        stats_rank.kills_match = checkNumber(
            profile[profile.indexOf('Kills/match') + 1]
        )
        stats_rank.kills_min = checkNumber(
            profile[profile.indexOf('Kills/min') + 1]
        )
...
*/
    ...
    deathmatch: async function (platform, name) {
        ...
        stats_deathmatch.kills = checkNumber(
            profile[profile.indexOf('Kills') + 1]
        )
        stats_deathmatch.deaths = checkNumber(
            profile[profile.indexOf('Deaths') + 1]
        )
        stats_deathmatch.win_ = profile[profile.indexOf('Win %') + 1]
        stats_deathmatch.wins = checkNumber(
            profile[profile.indexOf('Wins') + 1]
        )
        stats_deathmatch.losses = checkNumber(
            profile[profile.indexOf('Losses') + 1]
        )

        stats_deathmatch.abandons = checkNumber(
            profile[profile.indexOf('Abandons') + 1]
        )
        stats_deathmatch.matches = String(
            parseInt(stats_deathmatch.wins) +
                parseInt(stats_deathmatch.losses) +
                parseInt(stats_deathmatch.abandons)
        )
        stats_deathmatch.kills_match = checkNumber(
            profile[profile.indexOf('Kills/Match') + 1]
        )
    ...
    operator: async function (platform, name, operator) {
        ...
        stats_operator.melee_kills = checkNumber(
            profile[board.indexOf('Melee Kills')]
        )
